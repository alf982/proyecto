...
class ArticuloController extends Controller implements HasMiddleware {
    ...
    public function index(Request $request) {
        $q = Articulo::with('almacen')
            ...
            ->orderBy('nombre')->paginate(25)->withQueryString();
        $resumen = [
    ...
    public function store(Request $request) {
        $request->validate(['codigo'=>'required|unique:articulos,codigo','nombre'=>'required','tipo'=>'required|in:bien,servicio,material,equipo']);
        Articulo::create([...$request->only(['codigo','nombre','descripcion','tipo','unidad_medida','precio_referencia','stock_minimo','categoria','almacen_id']),'stock_actual'=>$request->stock_inicial ?? 0,'activo'=>true,'creado_por'=>auth()->id()]);
        return redirect()->route('compras.articulos.index')->with('success','Artículo creado correctamente.');
    }
    ...
    public function update(Request $request, Articulo $articulo) {
        $request->validate(['codigo'=>'required|unique:articulos,codigo,'.$articulo->id,'nombre'=>'required']);
        $articulo->update($request->only(['codigo','nombre','descripcion','tipo','unidad_medida','precio_referencia','stock_minimo','categoria','almacen_id']));
        return redirect()->route('compras.articulos.show',$articulo)->with('success','Artículo actualizado.');
    }
    // Ajuste de inventario manual
    public function ajustar(Request $request, Articulo $articulo) {
        $request->validate(['tipo'=>'required|in:entrada,salida,ajuste','cantidad'=>'required|numeric|min:0.01','concepto'=>'required|string']);
